...
到第二輪,agent 在為它設計的題組上 **18/20 vs 9/20,翻對 9 / 翻錯 0,p = 0.004:這是專案裡 agent 第一次顯著贏過固定管線**。
...
1. **檢索對問法極度敏感。** 同一件事,「減肥廣告違反哪一條?」reranker 0.91 過閘門;agent 濃縮成關鍵字「減肥廣告」只剩 0.27;
   整句三連問只剩 0.17。固定管線送整句、agent 送關鍵字,送進檢索的都是壞查詢
...
案例檢索相似度過門檻(0.58,實測相關 0.60–0.66、無關 0.45–0.56——邊界很窄,是已知限制)才算依據,且算進 grounded,固定管線與 agent 同一規則:法規那一跳沒過閘門、但案例可信,就只憑案例作答(「有哪些業者因為宣稱減肥被罰?」原本查到 3 筆正確案例卻整題拒答);
