...
confidence gate → cloud LLM**, plus a bounded tool-calling agent (`/ask_agent`). The point is not the feature list:
...
|---|---|---|
| Does two-stage retrieval (dense → rerank) help? | **Depends on the reranker.** `bge-reranker-base` gives no significant gain (p = 0.82); `bge-reranker-v2-m3` does (Recall@1 +0.12 [+0.05, +0.19], p = 0.004) | [§1](#1-retrieval-two-stage-is-not-automatically-better) |
...
| Is "let the LLM decide" better than a fixed retry? | **No.** Single-hop 99 vs 100/108; even on the 20 multi-hop questions built for it, 8 vs 10 (p = 0.69). Its only win is the related-articles tool the baseline lacks (3/3) | [§3](#3-agent-fixed-retry-vs-tool-calling-harness), [§5](#5-multi-hop-when-the-agent-actually-matters) |
| What does each harness boundary actually block? | Ablation says: only the drift check (31 → 27/32 when off); the forced refusal and the citation check caught nothing on this set — insurance, not gain | [§4](#4-harness-ablation-why-each-boundary-exists) |
| Is the eval set big enough? | 24 hand-written → **108** (+84 LLM-generated, human-reviewed), plus 8 hard, 20 out-of-domain, 20 multi-hop | [eval/](eval/) |
...
flowchart LR
    ...
    subgraph router["/query: single entry point"]
        ...
        RT -- ad_review --> REV[/review<br/>ad audit/]
        RT -- multi_hop --> Q2
    end

    subgraph pipe["/ask: fixed pipeline"]
...
`/ask` is deterministic (low confidence → exactly one rewrite-and-retry). `/ask_agent` wraps the same retrieval as
tools and lets the LLM drive control flow; the harness enforces boundaries in code — **a prompt is a request, code is a
guarantee.**
...
was written up as a clear win — it was one extra correct question. On 108 questions it flips 10 right and 8 wrong,
...
| Set | Fixed retry (`/ask`) | Tool-calling agent (`/ask_agent`) | flipped right / wrong | sign test p |
|---|---|---|---|---|
| main (108) | **100/108 = 0.926 [0.870, 0.972]** | 99/108 = 0.917 [0.861, 0.963] | 0 / 1 | 1.000 |
| hard (8) | 6/8 = 0.750 [0.500, 1.000] | 6/8 = 0.750 [0.500, 1.000] | 0 / 0 | 1.000 |

- **The fixed rewrite-and-retry does help at n = 108**: 8 retries triggered, 4 rescued. The earlier negative result
...
- **The agent ties on single-hop questions; it is not better.** Only 3 of 108 questions used more than one tool call;
  the literal-anchor drift check intervened on 25; latency is 31.5 s/question (the fixed pipeline measured ~13 s on the same
  machine earlier). The earlier "31/32 beats 30/32" conclusion does not survive n = 108 (99 vs 100).
- Both systems have **confidently-wrong** cases (fixed retry: 5 + 2) that no confidence mechanism can detect.
- Conclusion: the agent's extra freedom (choosing its own query strings and how many calls to make) buys latency, not
  ...
  to the agent and everything else to the fixed pipeline (§6).

### 4. Harness ablation: why each boundary exists

Each `/ask_agent` boundary switched off one at a time, same questions (24 hand-written + 8 hard for hit rate; 20
...
| no_grounding | no forced refusal when ungrounded | 30/32 = 0.938 [0.844, 1.000] | **0/20** | 19.3 |
| no_drift_check | no literal-query consistency anchor | **27/32 = 0.844 [0.719, 0.969]** | 0/20 | 11.4 |
| temp_0.1 | decision temperature back to 0.1 | 31/32 = 0.969 [0.906, 1.000] | 0/20 | 18.3 |

Three honest conclusions — one positive, one "redundant", one "not measurable in a single run":

- **The drift check earns its keep**: switching it off loses 4 questions (31 → 27), exactly the query-drift cases
...
- **temperature = 0 cannot be shown in one run**: 0.1 also scores 31/32. The earlier "same question, different result on
...
passed the gate and got an answer, 89 answers cite article numbers, and **0/100** cited an article absent from the
...
| Component | baseline (`/ask` logic: fixed retry + keyword-triggered case lookup) | tool-calling agent |
|---|---|---|
| reg_hit (right regulation) | **14/20 = 0.700 [0.500, 0.900]** | 10/20 = 0.500 [0.300, 0.700] |
...
Flipped 2 right / 4 wrong, sign test p = 0.688; the agent averaged 2.15 tool calls and used more than one on 18/20
...
- For the case hop, `/ask`'s keyword trigger (query cases if the question mentions penalties / cases) does as well as the
  LLM deciding to (16 vs 15).
- For the regulation hop the agent loses 4 questions: the LLM's condensed queries drift more in multi-step settings
...
### 6. Router: a new failure point, measured
...
`meta` carries `confident` / `used_retry` / `unsupported_citations`; `/ask_agent` additionally returns `trace`
(tool, query, confidence, drift intervention per step) and `usage` (tokens, seconds, stop reason).
...
```
app/        FastAPI + retrieval/agent logic (retrieval, corrective_retrieval, agentic_retrieval, agent, agent_tools, verifier)
