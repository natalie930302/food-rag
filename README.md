...
| 「讓 LLM 自主決策」比固定重試好嗎? | **單跳沒有**(99 vs 99/108)。為它設計的 20 題 multi-hop 第一次量是輸的(6 vs 10);兩輪修的都是 harness 與工具契約、不是 agent,修完 **17 vs 9**(翻對 8 / 翻錯 0,p=0.008,n=20),且有 baseline 做不到的關聯法條查詢 | [§Agent](#3-agent固定重試-vs-tool-calling-harness)、[§Multi-hop](#5-multi-hopagent-什麼時候才真的有用診斷修正再量) |
...
- 結論:agent 多出來的自由度(自己決定查詢字串、查幾次)在單跳問題上沒有換到準確率,只換到延遲。它的價值只能在
  需要多步的問題上量(§5)——第一次量沒有贏,追出根因、兩輪修好邊界與工具契約後才顯著領先(17 vs 9,p=0.008)。這是 `/query` 只把 multi-hop
...
修正一:翻對 5 / 翻錯 1,p = 0.219。修正二:**翻對 8 / 翻錯 0,p = 0.008**;agent 平均 2.55 次工具呼叫、20/20 題用了 >1 次;
延遲 baseline 14.0 秒 / agent 27.8 秒。(baseline 四輪分別 10、9、9、9。)

**誠實的結論**:兩輪修正都不是「讓 agent 更聰明」,而是修它的 harness 與工具契約。第一輪補齊工具重試、強制先查法規(法規 8 → 15);
第二輪是為 §8 複合問題做的——案例相似度過門檻才算依據、關鍵字補成問句——多跳題組的提升是副產品(法規 15 → 17、案例 15 → 17)。
到第二輪,agent 在為它設計的題組上 **17/20 vs 9/20,翻對 8 / 翻錯 0,p = 0.008:這是專案裡 agent 第一次顯著贏過固定管線**。
仍要保留:n = 20,CI 寬 ±20 點;baseline 四輪在 9–10 之間抖動;代價是 2 倍延遲。「顯著」在 n = 20 上站得住,但要當成結論還是得擴到 50–60 題。
...
案例檢索相似度過門檻(0.58,實測相關 0.60–0.66、無關 0.45–0.52)才算依據,且算進 grounded;
...
逐題翻對 4 / 翻錯 1,exact sign test p = 0.375。翻對的四題有三題是修正前「整題拒答」;翻錯的一題是拆子問題後「沒標會怎樣」那一半沒檢索到罰則,誠實寫了「沒找到」(修正前整句檢索碰巧撈到)。代價是延遲增加(17.6 → 23.6 秒/題,1.3 倍):子問題各自檢索、各自改寫重試。重跑 20 題 multi-hop:agent 13 → **17/20**(§5,案例門檻與關鍵字→問句同樣作用在多跳題上);重跑 108 題單跳:101 → 101/108、OOD 拒答 20/20,沒有退步。
