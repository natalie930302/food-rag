...
- 一套完整可重現的評估流程:108 題主評估集(手寫/合成分開報告)、hard set、out-of-domain、multi-hop 四組題目,`eval/stats.py` 提供 bootstrap CI、paired bootstrap、精確符號檢定,`make eval` 一鍵產出 `eval/RESULTS.md`
...
本文用詞統一如下:**意圖**指分流層對輸入的判定(問規定 / 查案例 / 審稿 / 多步,四種);**路徑**指判定後實際執行的程式(固定路徑 / 審稿路徑 / agent 路徑,三條,前兩種意圖共用固定路徑);**檢索管線**專指「向量粗篩 → 實體加權 → reranker 重排 → 信心閘門 → 改寫重查」五步,固定路徑直接呼叫、agent 路徑將其包成 `search_regulations` 工具;**harness** 指三條路徑共用的邊界層。
...
### 3.2 固定路徑(regulation_qa / case_lookup)
...
2026/09 multi-hop 評估後另補兩道邊界(5.5 節):`search_regulations` 工具內建與固定路徑相同的改寫重試;LLM 未呼叫任何法規檢索即欲作答時,harness 以原始問題強制補查一次。兩者皆可關閉以供消融。決策 temperature 固定為 0(早期發現 0.1 會讓邊界名次附近的題目重跑結果不一致)。每步記錄為 `ToolCallRecord`(工具、參數、信心、chunk ids、是否觸發 drift 介入),`scripts/replay_trace.py` 可離線逐步回放。

### 3.4 `/query`:先分流,再決定用多重的機制

5.3 節顯示 agent 在單跳問題上與固定路徑同樣準確但慢一倍,因此單一入口 `/query` 採 Adaptive-RAG [8] 的原則:先判斷意圖,只把需要多步檢索的問題交給 agent。路由分兩層——關鍵字規則(零成本、確定性,只在訊號很強時判定)→ gpt-4o-mini 結構化 JSON 分類(temperature 0,規則判不出來才呼叫)。四類意圖:`regulation_qa`、`case_lookup`(固定路徑,後者強制查案例)、`ad_review`(審稿)、`multi_hop`(tool-calling agent)。路由決定與理由隨回應回傳。
...
固定重試在 108 題上 8 題觸發、3 題救回,推翻早期 8 題 hard set「0 題救回」的負向結論。agent 在單跳問題上與固定重試無差異(108 題僅 3 題使用 >1 次工具;drift 檢查介入 25 題),延遲 20.5 秒/題(固定路徑約 11 秒);此為 5.5 節兩道修正上線後之重跑,單跳未退步。早期 32 題上「31/32 > 30/32」的結論在 108 題上不成立(99 vs 99)。兩系統各有 confidently wrong 案例(固定重試 6+1 題)。
...
20 題需要「法規 + 案例」或「法規 + 關聯法條」的問題,命中拆為三元件。第一次量(修正前):baseline(固定路徑 + 關鍵字觸發查案例)full_hit 10/20,agent 6/20(翻對 1/翻錯 5,p=0.219);agent 輸在法規一跳(8/20 vs 13/20),案例一跳兩者相當,關聯法條僅 agent 可做(3/3)。追查根因為兩個設計不對等:(a) 固定路徑的法規檢索含「LLM 改寫重查一次」,agent 的工具沒有、LLM 亦少自行重試;(b)「回答前至少查一次法規」只寫在 prompt,2/20 題 LLM 跳過。修正(工具內建重試;harness 在 LLM 未查法規即作答時強制補查一次,皆可關閉以供消融)後重量:

| 元件 | baseline | agent(修正前) | agent(修正一) | agent(修正二,5.9 節) |
|---|---|---|---|---|
...
修正一:翻對 5/翻錯 1,p=0.219,方向翻轉且提升集中於被診斷的法規一跳,但未達顯著。修正二(5.9 節為複合問題所做:案例相似度門檻、關鍵字補成問句)重量:翻對 9/翻錯 0,p=0.004,為本專案中 agent 首次顯著優於固定路徑;agent 平均 2.45 次工具呼叫,延遲(GPU 執行)baseline 14.0 秒、agent 27.8 秒。baseline 五次執行為 10、9、9、9、9。n=20 之下 CI 仍寬,結論之確立仍需擴大題組。本題組首次執行時另暴露 `search_violation_cases` 誤用法規 chunk 之 FAISS 索引的缺陷(案例命中 0/17),已修正並加入回歸測試。
...
使用者自由輸入「減肥廣告違反哪一條?罰多少?有沒有實際案例?」時系統整題拒答,而 agent 已查到 3 筆相關案例。診斷出三個程式碼層缺陷:(a) 檢索對問法敏感——同一主題單問句 reranker 0.91、關鍵字 0.27、三連問整句 0.17,固定路徑與 agent 送進檢索的都是壞查詢;(b) grounded 邊界只認法規檢索,案例與關聯法條被整題丟棄;(c) 規則層將「罰多少」視為案例訊號、對複合句仍直接判單跳。修正:複合問題拆子問題各自檢索合併;工具端將關鍵字補成問句;案例相似度 ≥ 0.58 方計為依據並納入 grounded(固定路徑與 agent 同一規則,法規未過閘門而案例可信時僅憑案例作答;無關問題實測最高 0.56,邊界窄為已知限制);關聯法條與案例納入引用驗證證據;部分無依據時逐部分標示而非整題拒答;混合訊號複合句交 LLM 路由。以 16 題複合問題(每題 2–3 部分)於修正前後量測:

| 版本 | full_hit | parts_hit | 整題拒答 | 秒/題 |
|---|---|---|---|---|
| 修正前 | 11/16 = 0.688 [0.438, 0.875] | 28/35 = 0.800 [0.657, 0.914] | 3 | 17.6 |
| 修正後 | 14/16 = 0.875 [0.688, 1.000] | 33/35 = 0.943 [0.857, 1.000] | 0 | 23.6 |

逐題翻對 4/翻錯 1,p=0.375;翻對者三題原為整題拒答,翻錯者為拆分後一子問題未檢索到罰則而誠實標示無資料。延遲由 17.6 增為 23.6 秒/題。此題組於修正後撰寫,量的是修正是否生效而非獨立驗證;其貢獻在於指出「一句一問」評估集看不見的失效模式,且再次顯示失效在 harness 與工具契約,不在 agent 的判斷。
...
**agent 的價值要用對的題目才量得到——而且第一次量出來的「輸」是可診斷的。** 單跳問題上 agent 平均只呼叫 1 次工具,與固定重試打平;多步題組第一次量 full_hit 6 vs 10,追查發現不是 agent 概念的問題,而是兩個設計不對等:工具缺少固定路徑有的改寫重試,「先查法規」只寫在 prompt 而未由程式碼保證。第一輪修正後 13 vs 9(p=0.219);第二輪(為複合問題所做的案例門檻與關鍵字→問句)後 18 vs 9(p=0.004)。兩輪都是修 harness 與工具契約而非 agent 本身,這也是本報告最主要的方法論結論:agent 的成敗取決於為它寫的邊界。保留:n=20、代價 2 倍延遲;它的確定價值仍是 baseline 做不到的關聯法條查詢。
