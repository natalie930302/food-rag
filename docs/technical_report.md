...
| hard(8) | 7/8 | 6/8 | 0/1 | 1.000 |

固定重試在 108 題上 8 題觸發、3 題救回,推翻早期 8 題 hard set「0 題救回」的負向結論。agent 在單跳問題上與固定重試無差異(108 題僅 3 題使用 >1 次工具;drift 檢查介入 24 題),延遲 18.6 秒/題(固定管線約 11 秒)。早期 32 題上「31/32 > 30/32」的結論在 108 題上不成立(99 vs 99)。兩系統各有 confidently wrong 案例(固定重試 6+1 題)。
...
|---|---|---|
| reg_hit | 13/20 = 0.650 [0.450, 0.850] | 8/20 = 0.400 [0.200, 0.600] |
...
翻對 1 / 翻錯 5,p = 0.219;agent 平均 2.25 次工具呼叫、19/20 題 >1 次。即使在為 agent 設計的題組上,agent 未勝過固定管線加關鍵字規則:案例一跳兩者相當;法規一跳 agent 輸 5 題(多步時 LLM 濃縮查詢的語意飄移放大,且 2 題跳過法規檢索);唯一無可取代者為關聯法條工具(3/3)。本題組首次執行時案例命中 0/17,追查發現 `search_violation_cases` 誤用法規 chunk 的 FAISS 索引——潛藏數月的真實缺陷,由多步題組暴露,已修正並加入回歸測試。
