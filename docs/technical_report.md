...
修正一:翻對 5/翻錯 1,p=0.219,方向翻轉且提升集中於被診斷的法規一跳,但未達顯著。修正二(5.9 節為複合問題所做:案例相似度門檻、關鍵字補成問句)重量:翻對 8/翻錯 0,p=0.008,為本專案中 agent 首次顯著優於固定管線;agent 平均 2.55 次工具呼叫,延遲 baseline 14.0 秒、agent 27.8 秒。baseline 四次執行為 10、9、9、9。n=20 之下 CI 仍寬,結論之確立仍需擴大題組。本題組首次執行時另暴露 `search_violation_cases` 誤用法規 chunk 之 FAISS 索引的缺陷(案例命中 0/17),已修正並加入回歸測試。
...
**agent 的價值要用對的題目才量得到——而且第一次量出來的「輸」是可診斷的。** 單跳問題上 agent 平均只呼叫 1 次工具,與固定重試打平;多步題組第一次量 full_hit 6 vs 10,追查發現不是 agent 概念的問題,而是兩個設計不對等:工具缺少固定管線有的改寫重試,「先查法規」只寫在 prompt 而未由程式碼保證。第一輪修正後 13 vs 9(p=0.219);第二輪(為複合問題所做的案例門檻與關鍵字→問句)後 17 vs 9(p=0.008)。兩輪都是修 harness 與工具契約而非 agent 本身,這也是本報告最主要的方法論結論:agent 的成敗取決於為它寫的邊界。保留:n=20、代價 2 倍延遲;它的確定價值仍是 baseline 做不到的關聯法條查詢。
