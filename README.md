...
| 「讓 LLM 自主決策」比固定重試好嗎? | **單跳沒有**(99 vs 99/108)。為它設計的 20 題 multi-hop 第一次量是輸的(6 vs 10);兩輪修的都是 harness 與工具契約、不是 agent,修完 **18 vs 9**(翻對 9 / 翻錯 0,p=0.004,n=20),且有 baseline 做不到的關聯法條查詢 | [§Agent](#3-agent固定重試-vs-tool-calling-harness)、[§Multi-hop](#5-multi-hopagent-什麼時候才真的有用診斷修正再量) |
...
| 評估集夠大嗎? | 24 題手寫 → **108 題**(+84 題 LLM 生成、人工審查),外加 8 題 hard、20 題 out-of-domain、20 題 multi-hop、16 題複合問題、**260 題國考單選題(官方答案)** | [eval/](eval/) |
| 使用者真的一句問三件事會怎樣? | 第一次:整題拒答。診斷出檢索對問法敏感、grounded 只認法規、規則層太貪三個程式碼缺陷,修後 16 題複合問題 full_hit 11 → **14**/16 | [§複合問題](#8-複合問題使用者一句問三件事整套系統一起失守診斷修正再量) |

## 名詞(全文統一)

| 詞 | 意思 | 有幾個 |
|---|---|---|
| **意圖(intent)** | 分流層對一次輸入的判定:問規定 `regulation_qa`、查案例 `case_lookup`、審稿 `ad_review`、多步 `multi_hop` | 4 |
| **路徑(path)** | 判定之後實際執行的程式:固定路徑、審稿路徑、agent 路徑(前兩種意圖走同一條固定路徑) | 3 |
| **檢索管線(retrieval pipeline)** | 「向量粗篩 → 實體加權 → reranker 重排 → 信心閘門 → 改寫重查」這五步;固定路徑直接用,agent 路徑把它包成 `search_regulations` 工具 | 1 |
| **harness** | 三條路徑共用的邊界層:執行軌跡、預算、拒答契約、引用驗證,全部由程式碼保證 | 1 |

| 意圖 | 例子 | 路徑 | 後面做什麼 |
|---|---|---|---|
| 問規定 | 真空包裝豆干要符合什麼規定? | 固定路徑 | (一句多問先拆子問題)→ 檢索管線 → 生成 → 引用驗證 |
| 查案例 | 有哪些業者因為宣稱減肥被罰? | 固定路徑 + 強制查案例 | 同上,另外到案例索引找最像的 3 筆;法規沒過閘門但案例相似度過門檻,就只憑案例作答 |
| 審稿 | (貼一段廣告文案) | 審稿路徑 | 掃高風險字眼 → 保證撈第 28 條 + 全庫補檢索 → 相似案例 → 三段報告 → 引用驗證 → 紅黃綠燈 |
| 多步 | 違反哪條?罰多少?有案例嗎? / 第 28 條跟哪些條文有關? | agent 路徑 | LLM 拿三個工具(查法規 = 檢索管線、查案例、查關聯法條)自己決定順序,最多 4 次;harness 管邊界 |
...
flowchart LR
    ...
    subgraph pipe["固定路徑(regulation_qa / case_lookup)"]
...
固定路徑是確定性的(信心不足就固定做一次改寫重試);agent 路徑把同一套檢索包成工具交給 LLM
...
然後**只把真的需要多步檢索的問題交給 agent**——單跳問題上 agent 跟固定路徑一樣準但慢一倍(§3),所以預設走便宜、
...
| 問題集 | 固定重試(固定路徑) | Tool-calling agent(agent 路徑) | 翻對 / 翻錯 | 符號檢定 p |
|---|---|---|---|---|
...
- **agent 在單跳問題上打平,沒有更好**:108 題裡只有 3 題用了 >1 次工具;字面錨定 drift 檢查介入了 25 題(LLM 改寫的查詢
  跟原始問題 top-1 不一致);延遲 20.5 秒/題(reranker 在 GPU;固定路徑約 11 秒)。數字為 §5 兩道修正上線後的重跑,單跳沒有退步。早期 32 題上「31/32 超越 30/32」的結論,
...
- 結論:agent 多出來的自由度(自己決定查詢字串、查幾次)在單跳問題上沒有換到準確率,只換到延遲。它的價值只能在
  需要多步的問題上量(§5)——第一次量沒有贏,追出根因、兩輪修好邊界與工具契約後才顯著領先(18 vs 9,p=0.004)。這是 `/query` 只把 multi-hop
  交給 agent、其餘一律走固定路徑的依據(§7)
...
**第一次量(修正前):agent 輸給固定路徑 + 關鍵字規則**

| 元件 | baseline(固定路徑 + 關鍵字觸發查案例) | tool-calling agent |
|---|---|---|
...
1. 固定路徑的法規檢索是「查一次 → 信心不夠就 LLM 改寫再查一次」;agent 的 `search_regulations` 工具只有「查一次」,
...
| 元件 | baseline | agent(修正前) | agent(修正一:工具重試 + 強制查法規) | **agent(修正二:§8 案例門檻 + 關鍵字→問句)** |
|---|---|---|---|---|
...
修正一:翻對 5 / 翻錯 1,p = 0.219。修正二:**翻對 9 / 翻錯 0,p = 0.004**;agent 平均 2.45 次工具呼叫、20/20 題用了 >1 次;
延遲 baseline 14.0 秒 / agent 27.8 秒(GPU 那一輪;最後一次重跑 reranker 在 CPU,命中數以它為準、延遲不採計)。(baseline 五輪分別 10、9、9、9、9。)

**誠實的結論**:兩輪修正都不是「讓 agent 更聰明」,而是修它的 harness 與工具契約。第一輪補齊工具重試、強制先查法規(法規 8 → 15);
第二輪是為 §8 複合問題做的——案例相似度過門檻才算依據、關鍵字補成問句——多跳題組的提升是副產品(法規 15 → 18、案例 15 → 17)。
到第二輪,agent 在為它設計的題組上 **18/20 vs 9/20,翻對 9 / 翻錯 0,p = 0.004:這是專案裡 agent 第一次顯著贏過固定路徑**。
仍要保留:n = 20,CI 寬 ±20 點;baseline 五輪在 9–10 之間抖動;代價是 2 倍延遲。「顯著」在 n = 20 上站得住,但要當成結論還是得擴到 50–60 題。
...
### 8. 複合問題:使用者一句問三件事,整套系統一起失守——診斷、修正、再量

108 題單跳、20 題多跳評估集全是「一句一問」的乾淨題目。使用者實測時一口氣問
「減肥廣告違反哪一條?罰多少?有沒有實際案例?」,系統整題拒答——而 agent 明明查到了 3 筆減肥裁罰案例。
追出來不是模型笨,是三個我寫的環節在複合句上失守,單跳評估集一題都量不到:

1. **檢索對問法極度敏感。** 同一件事,「減肥廣告違反哪一條?」reranker 0.91 過閘門;agent 濃縮成關鍵字「減肥廣告」只剩 0.27;
   整句三連問只剩 0.17。固定路徑送整句、agent 送關鍵字,送進檢索的都是壞查詢
2. **grounded 邊界只認法規檢索。** 案例查到了、關聯法條查到了,法規那一跳沒過閘門就整題覆寫成拒答
3. **規則層太貪。** 「被罰多少錢」被當成找案例;「哪條 + ?」不管句子裡幾個問號都直接判單跳,LLM 路由沒機會看

修法全部在程式碼層(`app/decompose.py` 新增;`router` / `agent_tools` / `agent` / `handlers` / `harness` 各改一處):
複合問題先拆子問題各自檢索再合併(子問題缺主詞時用一次 LLM 補);工具端收到關鍵字就補成問句再查;
案例檢索相似度過門檻(0.58,實測相關 0.60–0.66、無關 0.45–0.56——邊界很窄,是已知限制)才算依據,且算進 grounded,固定路徑與 agent 同一規則:法規那一跳沒過閘門、但案例可信,就只憑案例作答(「有哪些業者因為宣稱減肥被罰?」原本查到 3 筆正確案例卻整題拒答);
關聯法條與有信心的案例列入引用驗證的證據集合(原本 agent 查到的關聯條號會被驗證器當成瞎掰而砍掉);
「部分找不到」寫在該部分、不再整題拒答;規則層把混合訊號的複合句交給 LLM 路由。

**新評估集 `eval/compound_questions.json`(16 題,每題 2–3 個部分各自判定,全部過才算 full_hit)**,修正前後同一組題:

| 版本 | full_hit | parts_hit | 整題拒答 | 路由分佈 | 平均 LLM 呼叫 | 秒/題 |
|---|---|---|---|---|---|---|
| 修正前(00317fa) | 11/16 = 0.688 [0.438, 0.875] | 28/35 = 0.800 [0.657, 0.914] | 3 | multi_hop 9, regulation_qa 7 | 3.0 | 17.6 |
| **修正後** | **14/16 = 0.875 [0.688, 1.000]** | **33/35 = 0.943 [0.857, 1.000]** | 0 | multi_hop 8, regulation_qa 8 | 2.94 | 23.6 |

逐題翻對 4 / 翻錯 1,exact sign test p = 0.375。翻對的四題有三題是修正前「整題拒答」;翻錯的一題是拆子問題後「沒標會怎樣」那一半沒檢索到罰則,誠實寫了「沒找到」(修正前整句檢索碰巧撈到)。代價是延遲增加(17.6 → 23.6 秒/題,1.3 倍):子問題各自檢索、各自改寫重試。重跑 20 題 multi-hop:agent 13 → **18/20**(§5,案例門檻與關鍵字→問句同樣作用在多跳題上);重跑 108 題單跳:101 → 100/108(1 題,在抖動範圍內)、OOD 拒答 20/20,沒有退步。

**誠實的結論**:這組題目是修完才寫的,量的是「修好了沒」,不是獨立驗證;16 題的 CI 很寬。但它補上了一個評估集的盲點——
乾淨的單問題目把三個真實失效模式全部藏住了,而使用者第一次自由輸入就踩到。教訓跟 §5 一樣:
**agent 的判斷層沒壞(三個工具都叫對了),壞的是我為它寫的邊界與工具契約**;prompt 裡寫「請給完整問句」沒有用,要工具端自己補。
...
app/                 FastAPI + 檢索/agent 邏輯
  ...
  handlers.py          三條執行路徑(固定路徑 / 審稿 / agent),共用同一個 RunContext
